...
let client = new pg.Client({ connectionString: connectionString,
 ...
 app.get('/healthz', function(req, res) {
   const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    ...
    console.log("/healthz is pinged from ip: " + ip + " source: " +req.query.source)
     getWords()
     .then(function(words) {
         res.send(words);
     })
    ...
    //      if (err) {
    //        console.log("/healthz db connection failed\n" + err.stack)
    //        res.status(500).send(err);
    //      }else{
    //        client.query('SELECT * FROM words ORDER BY word ASC', (err, result) => {
    //          done()
    //          if (err) {
    //            console.log("/healthz db select query failed\n" + err.stack)
    //            res.status(500).send(err);
    //          } else {
    //            res.send(result);
    //          }
